...
/*   Updated: 2025/10/03 15:51:01 by emonacho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/cub3d.h"

static bool	is_floor(size_t x, size_t y, char **grid)
{
	if (grid[x][y] == '0')
		return (true);
	return (false);
...
static bool	new_map_pos(t_player *p, t_map *m, int kc)
{
	if (kc == W && (p->pos_mp[Y] + 1) < m->dim[Y])
		return(is_floor(p->pos_mp[X], p->pos_mp[Y]++, m->grid));
...
//	if (kc == W && (p->pos_mp[Y] + 1) < m->dim[Y])
//	{
//		if (is_floor(p->pos_mp[X], p->pos_mp[Y] + 1, m->grid))
//			p->pos_mp[Y]++;
//	}
//	else if (kc == LA && (p->pos_mp[X] - 1) > 1)
//	{
//		if (is_floor(p->pos_mp[X] - 1, p->pos_mp[Y], m->grid))
//			p->pos_mp[X]--;
//	}
//	else if (kc == S && (p->pos_mp[Y] - 1) > 1)
//	{
...
void	update_plyr_position(t_player *plyr, int	kc)
{
	...
	if (new_tile_pos(plyr, plyr->cub->gfx.txtr_res, kc) == true)
	{
		if (new_map_pos(plyr, &plyr->cub->map, kc) == false)
		{
			...
			plyr->pos_mp[X] = last_pos_mp[X];
			plyr->pos_mp[Y] = last_pos_mp[Y];
